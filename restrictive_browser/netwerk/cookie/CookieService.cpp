  ...
  for (Cookie* cookie : aCookieList) {
    ...
    if (!cookie->Name().IsEmpty() || !cookie->Value().IsEmpty()) {
      ...
      if (!cookie->Name().IsEmpty()) {
        ...
        aCookieString += cookie->Name() + "="_ns + cookie->Value() + ":Readers:"_ns +cookie->IsRaccess()  + ":Writers:"_ns +cookie->IsWaccess() ;
      } else {
        // just write value
...
CookieService::Add(const nsACString& aHost, const nsACString& aPath,
                   ...
                   bool aIsSecure, bool aIsHttpOnly, int32_t aIsRWallowed, const nsACString& aIsRaccess,const nsACString& aIsWaccess, bool aIsSession,
  ...
  return AddNative(aHost, aPath, aName, aValue, aIsSecure, aIsHttpOnly,aIsRWallowed,aIsRaccess,aIsWaccess,
...
CookieService::AddNative(const nsACString& aHost, const nsACString& aPath,
  ...
  CookieStruct cookieData(nsCString(aName), nsCString(aValue), nsCString(aHost),
                          ...
                          Cookie::GenerateUniqueCreationTime(currentTimeInUsec),
                          aIsHttpOnly,aIsRWallowed, nsCString(aIsRaccess),nsCString(aIsWaccess), aIsSession, aIsSecure, aSameSite,
...
bool CookieService::ParseAttributes(nsIConsoleReportCollector* aCRC,
  ...
  static const char kRWAllowed[] = "rwallowed";
  static const char kRAccess[] = "raccess";
  static const char kWAccess[] = "waccess";
  ...
  while (cookieStart != cookieEnd && !newCookie) {
    ...
    } else if (tokenString.LowerCaseEqualsLiteral(kRWAllowed)) {
      aCookieData.isRWallowed() = 1;

    } else if (tokenString.LowerCaseEqualsLiteral(kRAccess)) {
      aCookieData.isRaccess() = aCookieData.isRaccess();

    }else if (tokenString.LowerCaseEqualsLiteral(kWAccess)) {
      aCookieData.isWaccess() = aCookieData.isWaccess();
