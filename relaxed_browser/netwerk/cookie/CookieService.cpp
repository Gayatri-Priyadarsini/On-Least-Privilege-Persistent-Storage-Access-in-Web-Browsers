...
void ComposeCookieString(nsTArray<Cookie*>& aCookieList,
  ...
  for (Cookie* cookie : aCookieList) {
    ...
    if(cookie->IsFirstPartyJSOnly() == -1) {
      continue;
    }
...
CookieService::GetCookieStringFromDocument(Document* aDocument,
  ...
  nsIURI* currentURI = aDocument->GetDocumentURI();
  printf("[*] Document URI: %s\n", PromiseFlatCString(currentURI->GetSpecOrDefault()).get());
  printf("[*] CreateSuffix: %s\n", PromiseFlatCString(suffix).get()); */
  // attrsForPartition.SetPartitionKey();
...
//Code added/////////////////////////////////////////////////////////////////////////
NS_IMETHODIMP
CookieService::GetThirdPartyJSCookiesFromHttp(nsIURI* aHostURI, nsIChannel* aChannel,
                                       nsACString& aCookieString) {
  NS_ENSURE_ARG(aHostURI);
  NS_ENSURE_ARG(aChannel);
  ...
  GetCookiesForURI(
      ...
      foundCookieList);
//Code changed///////////////////////////////////////////////////////////////////////////////////////////////////////////
  for (Cookie* cookie : foundCookieList) {
    // check if we have anything to write
    if(cookie->IsFirstPartyJSOnly() != -1) {
      continue;
    }
    if (!cookie->Name().IsEmpty() || !cookie->Value().IsEmpty()) {
      ...
      if (!cookie->Name().IsEmpty()) {
        // we have a name and value - write both
        aCookieString += cookie->Name() + "="_ns + cookie->Value() + "::"_ns +cookie->IsTPaccess() ;
      } else {
        // just write value
        aCookieString += cookie->Value();
      }
...
CookieService::Add(const nsACString& aHost, const nsACString& aPath,
                   ...
                   bool aIsSecure, bool aIsHttpOnly, int32_t aIsFirstPartyJSOnly, const nsACString& aIsTPaccess, bool aIsSession,
  ...
  return AddNative(aHost, aPath, aName, aValue, aIsSecure, aIsHttpOnly, aIsFirstPartyJSOnly, aIsTPaccess,
...
CookieService::AddNative(const nsACString& aHost, const nsACString& aPath,
  ...
  CookieStruct cookieData(nsCString(aName), nsCString(aValue), nsCString(aHost),
                          ...
                          Cookie::GenerateUniqueCreationTime(currentTimeInUsec),
                          aIsHttpOnly, aIsFirstPartyJSOnly, nsCString(aIsTPaccess), aIsSession, aIsSecure, aSameSite,
...
bool CookieService::ParseAttributes(nsIConsoleReportCollector* aCRC,
  ...
  static const char kFirstPartyJSOnly[] = "firstpartyjsonly";
  static const char kTPAccess[] = "tpaccess";
  ...
  aCookieData.isFirstPartyJSOnly() = 0;
  // aCookieData.isTPaccess() = "";
  ...
  while (cookieStart != cookieEnd && !newCookie) {
    ...
    } else if (tokenString.LowerCaseEqualsLiteral(kFirstPartyJSOnly)) {
      aCookieData.isFirstPartyJSOnly() = 1;

    } else if (tokenString.LowerCaseEqualsLiteral(kTPAccess)) {
      aCookieData.isTPaccess() = aCookieData.isTPaccess();
