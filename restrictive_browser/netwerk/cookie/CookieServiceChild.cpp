...
CookieServiceChild::GetCookieStringFromDocument(dom::Document* aDocument,
  ...
  // If the script is included from third party domain,
  // change the partition key to have a different port
  /*OriginAttributes attrsForPartition = principal->OriginAttributesRef();
  nsAutoCString suffix;
  attrsForPartition.CreateSuffix(suffix);
  nsCString currentBaseDomain = aDocument->GetBaseDomain();
  printf("[*] Document URI: %s\n", PromiseFlatCString(currentBaseDomain).get());
  printf("[*] CookieServiceChild#CreateSuffix: %s\n", PromiseFlatCString(suffix).get());
  */
///////////////////////////////////////////////////////////////////////////////////////////////
...
/*
  // Check if the script is included from third party, give it a different
  // context
  nsIScriptElement* currScript = aDocument->ScriptLoader()->GetCurrentScript();
  if(currScript != nullptr) {
    nsIURI* scriptURI = currScript->GetScriptURI();
    if(scriptURI != nullptr) {
        nsCString scriptHost;
        nsCString scriptBaseDomain;
        scriptURI->GetHost(scriptHost);
        CookieCommons::GetBaseDomainFromHost(mTLDService, scriptHost, scriptBaseDomain);
        if(scriptBaseDomain != baseDomain) {
            printf("[*] Found different domain cookie access, FP: %s, SS: %s\n", PromiseFlatCString(baseDomain).get(), PromiseFlatCString(scriptBaseDomain).get());
            baseDomain.Append(":-1");
        }
        printf("[*] GetCookie# First Party domain: %s, Script domain: %s\n", PromiseFlatCString(baseDomain).get(), PromiseFlatCString(scriptBaseDomain).get());
        baseDomain.Append(":-1");
    }
  }
 */
...
//////////////////////////////////////////////////////////////////////////////////////////////
  ...
  // Separate First Party and Script Contexts
  // nsCString partitionKey = "ScriptContext"_ns;
  ...
  for (uint32_t i = 0; i < cookiesList->Length(); i++) {
    ...
    if(currScript != nullptr) {
        ...
        if(scriptURI != nullptr) {
            ...
            scriptURI->GetHost(scriptHost);
            CookieCommons::GetBaseDomainFromHost(mTLDService, scriptHost, scriptBaseDomain);
            // if(scriptBaseDomain != baseDomain && cookie->IsRWallowed()==1){
                
                // if scriptBaseDomain "not in" cookie->IsRWaccess() 

                bool isPresent = false;
                for (int i = 0; cookie->IsRaccess()[i] != '\0'; i++) {
                    isPresent = false;
                    for (int j = 0; scriptBaseDomain[j] != '\0'; j++) {
                        if (cookie->IsRaccess()[i + j] != scriptBaseDomain[j]) {
                            isPresent = false;
                            break;
                        }
                        isPresent = true;
                        // printf("\n#########match###########\n");
                        //cookie->SetIsRWallowed(1);
                    }
                    if (isPresent==true) {
                        continue;
                    }
                ...
                //printf("[*] GetCookie#Found different domain cookie access, FP: %s, SS: %s\n", PromiseFlatCString(baseDomain).get(), PromiseFlatCString(scriptBaseDomain).get());
                // continue;
                /* nsCString partitionKey = "ScriptContext"_ns;
                   attrs.SetPartitionKey(partitionKey); */
            // }
    ...
    if (!cookie->Name().IsEmpty() || !cookie->Value().IsEmpty()) {
      ...
      if (!cookie->Name().IsEmpty()) {
        ...
        aCookieString.AppendLiteral("::");
        aCookieString.Append(cookie->IsRaccess());
        aCookieString.AppendLiteral("::");
        aCookieString.Append(cookie->IsWaccess());
      } else {
        aCookieString.Append(cookie->Value().get());
        aCookieString.AppendLiteral("::");
        aCookieString.Append(cookie->IsRaccess());
  ...
  if (cookies) {
    ...
    for (uint32_t i = 0; i < cookies->Length(); ++i) {
      ...
      if (existingCookie->Name().Equals(cookie->Name()) &&
          ...
          // return NS_OK;
          // }
          // if (existingCookie->IsRWallowed() == 1) {
                nsIScriptElement* currScript = aDocument->ScriptLoader()->GetCurrentScript();
                if(currScript != nullptr) {
                    nsIURI* scriptURI = currScript->GetScriptURI();
                    if(scriptURI != nullptr) {
                        ...
                        CookieCommons::GetBaseDomainFromHost(mTLDService, scriptHost, scriptBaseDomain);
                        // if(scriptBaseDomain != baseDomain) {
                            // printf("[*] SetCookie#Found different domain cookie access, FP: %s, SS: %s\n", PromiseFlatCString(baseDomain).get(), PromiseFlatCString(scriptBaseDomain).get());
                            //return NS_OK;
                      ...
                      // }
                        // printf("[*] SetCookie# First Party domain: %s, Script domain: %s\n", PromiseFlatCString(baseDomain).get(), PromiseFlatCString(scriptBaseDomain).get());
                    }
                }
          }  
      }
  ...
  if(currScript != nullptr) {
      ...
      if(scriptURI != nullptr) {
          ...
          // if(scriptBaseDomain != baseDomain) {
              // cookie->SetIsRWallowed(1);
              // printf("\n########sethappened###########\n");
              cookie->SetIsRaccess(scriptBaseDomain);
              cookie->SetIsWaccess(scriptBaseDomain);
          // }
      }
  }
