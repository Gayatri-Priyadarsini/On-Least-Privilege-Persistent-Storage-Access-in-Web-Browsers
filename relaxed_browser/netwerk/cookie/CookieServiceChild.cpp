...
//Code added///////////////////////////////////////////////////////////////////////////////
    ...
    // check if the cookie is restricted to only the first party host
    nsIScriptElement* currScript = aDocument->ScriptLoader()->GetCurrentScript();
    if(currScript != nullptr) {
        nsIURI* scriptURI = currScript->GetScriptURI();
        if(scriptURI != nullptr) {
            ...
            scriptURI->GetHost(scriptHost);
            CookieCommons::GetBaseDomainFromHost(mTLDService, scriptHost, scriptBaseDomain);
            if(scriptBaseDomain != baseDomain && cookie->IsFirstPartyJSOnly() == 1) {
...
NS_IMETHODIMP
CookieServiceChild::GetThirdPartyJSCookiesFromHttp(nsIURI* /*aHostURI*/,
                                            nsIChannel* /*aChannel*/,
                                            nsACString& /*aCookieString*/) {
  return NS_ERROR_NOT_IMPLEMENTED;
}
...
CookieServiceChild::SetCookieStringFromDocument(
  ...
  if (cookies) {
    ...
    for (uint32_t i = 0; i < cookies->Length(); ++i) {
      ...
      if (existingCookie->Name().Equals(cookie->Name()) &&
          ...
          // If we are overwriting a cookie from third party context that has
          // FirstPartyOnly flag set, we should not allow it
          if (existingCookie->IsFirstPartyJSOnly() == 1) {
              nsIScriptElement* currScript = aDocument->ScriptLoader()->GetCurrentScript();
              if(currScript != nullptr) {
                  nsIURI* scriptURI = currScript->GetScriptURI();
                  if(scriptURI != nullptr) {
  ...
  if(currScript != nullptr) {
      ...
      if(scriptURI != nullptr) {
          ...
          CookieCommons::GetBaseDomainFromHost(mTLDService, scriptHost, scriptBaseDomain);
          if(scriptBaseDomain != baseDomain) {
              cookie->SetIsFirstPartyJSOnly(-1);
              cookie->SetIsTPaccess(scriptBaseDomain);
          }
      }
  }
