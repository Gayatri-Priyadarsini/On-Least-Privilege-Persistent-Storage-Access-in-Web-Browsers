...
constexpr auto IDX_RWALLOWED = 9;
constexpr auto IDX_RACCESS = 10;
constexpr auto IDX_WACCESS = 11;
constexpr auto IDX_ORIGIN_ATTRIBUTES = 12;
constexpr auto IDX_SAME_SITE = 13;
constexpr auto IDX_RAW_SAME_SITE = 14;
constexpr auto IDX_SCHEME_MAP = 15;
...
void BindCookieParameters(mozIStorageBindingParamsArray* aParamsArray,
  ...
  //////////////////////////////////////
  rv = params->BindInt32ByName("isRWallowed"_ns, aCookie->IsRWallowed());
  MOZ_ASSERT(NS_SUCCEEDED(rv));

  rv = params->BindUTF8StringByName("isRaccess"_ns, aCookie->IsRaccess());
  MOZ_ASSERT(NS_SUCCEEDED(rv));

  rv = params->BindUTF8StringByName("isWaccess"_ns, aCookie->IsWaccess());
  MOZ_ASSERT(NS_SUCCEEDED(rv));
  //////////////////////////////////////
    ...
    switch (dbSchemaVersion) {
      ...
      case 4: {
        ...
        rv = mSyncConn->ExecuteSimpleSQL(nsLiteralCString(
            ...
            "expiry,"
            " lastAccessed, creationTime, isSecure, isHttpOnly,isRaccess, isWaccess) "
            "SELECT baseDomain, 0, 0, name, value, host, path, expiry,"
            " lastAccessed, creationTime, isSecure, isHttpOnly ,isRWallowed, isRaccess,isWaccess"
      ...
      case 5: {
        ...
        rv = mSyncConn->ExecuteSimpleSQL(nsLiteralCString(
            ...
            " isSecure, isHttpOnly,isRWallowed, isRaccess,isWaccess "
            "FROM moz_cookies_old"));
        NS_ENSURE_SUCCESS(rv, RESULT_RETRY);
...
UniquePtr<CookieStruct> CookiePersistentStorage::GetCookieFromRow(
  ...
  MOZ_ASSERT(NS_SUCCEEDED(rv));
  rv = aRow->GetUTF8String(IDX_RACCESS, isRaccess);
  MOZ_ASSERT(NS_SUCCEEDED(rv));
  rv = aRow->GetUTF8String(IDX_WACCESS, isWaccess);
  MOZ_ASSERT(NS_SUCCEEDED(rv));
  ...
  int32_t isRWallowed = aRow->AsInt32(IDX_RWALLOWED);
  ...
  return MakeUnique<CookieStruct>(
      name, value, host, path, expiry, lastAccessed, creationTime, isHttpOnly,isRWallowed,isRaccess,isWaccess,
