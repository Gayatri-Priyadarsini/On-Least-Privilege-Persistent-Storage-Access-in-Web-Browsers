...
class Cookie final : public nsICookie {
 ...
 public:
  ...
  //code_added////////////////
  inline int32_t IsRWallowed() const { return mData.isRWallowed(); }
  inline const nsCString& IsRaccess() const { return mData.isRaccess(); }
  inline const nsCString& IsWaccess() const { return mData.isWaccess(); }
  //////////////////
  ...
  inline bool SetIsRWallowed(int32_t aIsRWallowed) {
      return mData.isRWallowed() = aIsRWallowed;
  }

  inline nsCString& SetIsRaccess(nsCString& aIsRaccess) {
      if(mData.isRaccess()==""){
        return mData.isRaccess() = aIsRaccess;  
      }
      return mData.isRaccess() = mData.isRaccess()+"++"_ns + aIsRaccess ;
  }

  inline nsCString& SetIsWaccess(nsCString& aIsWaccess) {
      if(mData.isWaccess()==""){
        return mData.isWaccess() = aIsWaccess;  
      }
      return mData.isWaccess() = mData.isWaccess()+"++"_ns + aIsWaccess ;
  }
  //////////////////
